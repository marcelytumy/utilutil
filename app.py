...
def convert_to_mp4(file_paths, progress_callback=None):
    ...
    for i, file_path in enumerate(file_paths):
        ...
        output = base + '.mp4'
        try:
            proc = subprocess.Popen(['ffmpeg', '-i', file_path, '-c:v', 'libx264', '-c:a', 'aac', output], 
            ...
            print(f"[Convert to MP4] Converted {file_path} to {output}")
...
def compress_video(file_paths, progress_callback=None):
    ...
    for i, file_path in enumerate(file_paths):
        ...
        try:
            proc = subprocess.Popen(['ffmpeg', '-i', file_path, '-c:v', 'libx264', '-crf', '28', '-c:a', 'aac', output], 
                                    stderr=subprocess.PIPE, text=True)
            for line in proc.stderr:
                if 'time=' in line:
                    match = re.search(r'time=(\d+):(\d+):(\d+\.\d+)', line)
            ...
            if progress_callback:
                progress_callback(min(current_offset / total_duration * 100, 100))
            print(f"[Compress Video] Compressed {file_path} to {output}")
        except subprocess.CalledProcessError as e:
...
def translate_text(text, hwnd):
    try:
        ...
        if lang == 'en':
            ...
            import deepl
            auth_key = "83cd4d37-5f46-41ea-bcff-938988aaa54f:fx"  # Replace with your actual DeepL API key
